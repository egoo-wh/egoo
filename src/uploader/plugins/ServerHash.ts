...
import { UploadInfo, QueryInfo } from "../Uploader";
...
export default class ServerHash {
  ...
  constructor(uploader: Uploader) {
    ...
    uploader.hooks.query.tapPromise(
      "ServerHashPlugin",
      (queryInfo: QueryInfo, uploadInfo: UploadInfo) => {
  ...
  save(uinfo) {
    ...
    if (fileInfo && fileInfo.assets) {
      return fs
        .writeFile(fileInfo.filePath, JSON.stringify(fileInfo.assets))
        .then(() => {
          fileInfo.saved = true;
  ...
  clear() {
    ...
    const all = Object.values(this.fileInfos).map((info) => {
      if (info.saved && info.filePath) {
        return fs.unlink(info.filePath);
      } else {
        return Promise.resolve();
      }
    });
  ...
  generateHash(contents) {
    if (contents) {
      return (
        hashCfg.hashKey +
        crypto
          .createHash(hashCfg.hasher)
          .update(contents)
          .digest("hex")
          .slice(0, hashCfg.length)
      );
    }
