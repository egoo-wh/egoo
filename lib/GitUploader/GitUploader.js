...
class GitUploader {
	constructor(localPath, isSync){
		(!isSync?new Uploader(localPath, SERVER_REMOTE_PATH, false, false):Promise.resolve())
		.then((uploader)=>this.run(uploader))
		.catch((err)=>{
	...
	run(uploader) {
		...
		.then(()=>this.execGitCmd(ssh.client, 'commit -a -m "update"'))
	...
	execGitCmd(client, cmd) {
		log(LOG_NAME, gutil.colors.yellow('git -C /mnt/pages '+cmd));
