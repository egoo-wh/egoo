...
function showPrompt(props) {
	return new Promise((resolve, reject)=>{
		let p = _.reduce(props, (result, value)=>{
			if (_.isString(value)) {
				result[value] = {message: gutil.colors.yellow(value)+":", required: true};	
			} else if (_.isPlainObject(value)) {
				result[value['name']] = {message: gutil.colors.yellow(value['message']+":"), required: true};
			}
			return result;
		}, {});
	 
	 	prompt.message = gutil.colors.yellow('请输入')
	 	prompt.delimiter = '';
...
module.exports = {log, serverPathJoin, sfcall, showPrompt, bytesSizeFormat, getConfigURL};