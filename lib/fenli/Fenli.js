...
const log = (0, utils_1.logger)('Fenli');
...
class Fenli extends Handler_1.default {
    ...
    async run(aliases, url) {
        try {
            ...
            await this.start();
            log((0, utils_1.logMsg)('fenli success.', 'SUCCESS'));
        }
        catch (error) {
            log((0, utils_1.logMsg)('fenli fail.', 'ERROR'));
    ...
    async start() {
        return this.sources.reduce(async (promise, source) => {
            try {
                await promise;
                log((0, utils_1.logMsg)(`start ${(0, utils_1.logMsg)(source, 'PATH')}`, 'STEP'));
                await this.startOne(source);
            }
            catch (error) {
                return Promise.reject(error);
            }
            log((0, utils_1.logMsg)(`finish ${(0, utils_1.logMsg)(source, 'PATH')}`, "STEP"));
    ...
    startOne(source) {
        ...
        return (0, asyncs_1.walkFile)(source, ({ filePath, type }) => {
            let _d = path.join(dest, path.relative(source, filePath));
            // console.log(_d);
            if (type === 'file') {
                let pi = this.patchInstaller.detectAndAdd(filePath, _d);
                if (pi) {
                    log((0, utils_1.logMsg)(`replace ${(0, utils_1.logMsg)(path.relative(source, filePath), 'PATH')} > ${(0, utils_1.logMsg)(path.relative(source, _d), 'PATH')}`));
                    return this.patchInstaller.run(pi);
                }
                else {
                    log((0, utils_1.logMsg)(`cp ${(0, utils_1.logMsg)(path.relative(source, filePath), 'PATH')} > ${(0, utils_1.logMsg)(path.relative(source, _d), 'PATH')}`));
                    return (0, asyncs_1.cp)(filePath, _d);
                }
            }
            else {
                log((0, utils_1.logMsg)(`create dir ${(0, utils_1.logMsg)(path.relative(source, _d), 'PATH')}`));
                return (0, asyncs_1.cp)(filePath, _d);
            }
        });
    }
     ...
     */
    async loadFenliData() {
        log((0, utils_1.logMsg)('load fenli data.', 'STEP'));
        try {
            ...
            if (response.ok) {
                ...
                if (data && data['ret'] == 0) {
                    ...
                    log((0, utils_1.logMsg)('load fenli data error.', 'ERROR'));
