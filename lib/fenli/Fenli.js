 ...
 * 	- 复制并重命名（项目名后面加“分离后”）整个包	
	...
	- 去掉上述文件中绝对路径前的http(s):，以下情况除外：
		- 分享组件（TGMobileShare({shareImgUrl:https://...})），分享图地址的https不会去除。
		- css中svg data uri的形式(data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink')时，http也不会删除。
...
const isBase64URL = (url) => {
	return url.match(/data\:(.*?)base64,/g);
}
...
class Fenli  extends Handler{
			...
			if (type == 'file') {
				if (['.html', '.htm', '.shtml', '.inc'].indexOf(_extName) >= 0) {
					...
					return FileModifier.modify(filePath, _d, (res)=>{
						...
						res = res.replace(HTML_HTTP_REG, (match, p1, p2)=>{
							if (p1 && p1.indexOf("shareImgUrl") >= 0) {
								return match;
							} else {
								return "";
							}
					...
					return FileModifier.modify(filePath, _d, (res)=>{
						res = this.replaceURL(res, filePath);
						res = res.replace(CSS_HTTP_REG, (match, p1, p2)=>{
							if (p1 && p1.indexOf("xmlns") >= 0) {
								return match;
							} else {
								return "";
							}
	...
	replaceURL(res, filePath) {
		...
		res = res.replace(URL_REG, (match, p1, p2, p3, offset) => {
			if (!isBase64URL(match) && isRelativePath(p2)) {
