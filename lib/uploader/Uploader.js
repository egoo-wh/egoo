...
const log = utils_1.logger('Uploader');
/**
 * 上传列表单个信息
 */
class QueryInfo {
...
class Uploader extends Handler_1.default {
    ...
    async run(sources, mode, ignores, configForceReload) {
        try {
            ...
            await this.start();
            await this.close();
            log(utils_1.logMsg('publish success.', 'SUCCESS'));
            log(utils_1.logMsg('preview url: ' + this.getPreviewURLs().map(u => {
                return utils_1.logMsg(u, 'UNDERLINE');
            }).join(' , ')));
        }
        catch (error) {
            log(utils_1.logMsg('publish fail.', 'ERROR'));
    ...
    async start() {
        return this.uploads.reduce(async (promise, uinfo) => {
            try {
                await promise;
                log(utils_1.logMsg(`start query ${utils_1.logMsg(uinfo.source, 'PATH')}`, "STEP"));
                await this.hooks.beforeQuery.promise(uinfo);
                await this.buildQuery(uinfo);
                await this.hooks.afterQuery.promise(uinfo);
                log(utils_1.logMsg(`finish query ${utils_1.logMsg(uinfo.source, 'PATH')}`, "STEP"));
    ...
    async isSourcesValid(sources, remotePath) {
        ...
        return sources.reduce(async (promise, source) => {
            ...
            try {
                ...
                log(utils_1.logMsg(`上传路径:${source} 不正确`, 'ERROR'));
                throw error;
            }
            let info;
            if (stats.isDirectory()) {
                info = new UploadInfo(source, utils_1.serverPathJoin(remotePath, path.basename(source)), 'dir');
            }
            else {
    ...
    // 创建上传队列
    buildQuery(info) {
        return asyncs_1.walkFile(info.source, async ({ filePath, type }) => {
            ...
            const dest = utils_1.serverPathJoin(this.remotePath, basename, pathItems);
    ...
    uploadQuerys(sftp, querys) {
        log(utils_1.logMsg('start upload query.', 'STEP'));
        return new Promise((resolve, reject) => {
            this.uploadQuery(sftp, querys, 0)
                .then((finish) => {
                if (finish) {
                    log(utils_1.logMsg('finish upload query.', 'STEP'));
                    resolve();
                }
                else {
    ...
    uploadQuery(sftp, query, loadIndex) {
        ...
        let p;
        if (type === 'dir') {
            p = asyncs_1.sfcall(sftp.opendir, sftp, true, dest).then((dirExists) => {
                if (!dirExists) {
                    log(utils_1.logMsg('mkdir: ' + utils_1.logMsg(dest, 'PATH')));
                    // mkdir.
                    return asyncs_1.sfcall(sftp.mkdir, sftp, false, dest);
                }
                else {
                    return dirExists;
                }
                ...
                if (!dirExists) {
                    ...
                    log(utils_1.logMsg('chmod directory: ' + utils_1.logMsg(dest, 'PATH')));
                    return asyncs_1.sfcall(sftp.chmod, sftp, false, dest, '0777');
                }
            }).then(() => {
            ...
            p = asyncs_1.sfcall(sftp.open, sftp, true, dest, 'r').then((fileExists) => {
                // if file exist, delete it.
                if (fileExists) {
                    log(utils_1.logMsg('rm: ' + utils_1.logMsg(dest, 'PATH')));
                    return asyncs_1.sfcall(sftp.unlink, sftp, false, dest);
                }
                ;
            }).then(() => {
                // upload file.
                log(utils_1.logMsg('cp: ' + utils_1.logMsg(src, 'PATH') + ' > ' + utils_1.logMsg(dest, "PATH")));
                return asyncs_1.sfcall(sftp.fastPut, sftp, false, src, dest);
            }).then(() => {
                // upload complete. next.
                loadIndex++;
                return this.uploadQuery(sftp, query, loadIndex);
