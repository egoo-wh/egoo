...
class Uploader extends Handler_1.default {
    ...
    constructor() {
        ...
        this.hooks = {
            ...
            beforeUpload: new tapable_1.AsyncSeriesHook(['uinfo']),
            afterUpload: new tapable_1.AsyncSeriesHook(['uinfo']),
    ...
    async run(sources, mode, ignores, configForceReload) {
        try {
            ...
            log(utils_1.logMsg('preview url: ' + utils_1.logMsg(this.getPreviewURLs().join(','), 'UNDERLINE')));
        }
        catch (error) {
    ...
    uploadQuerys(sftp, querys) {
        log(utils_1.logMsg('query load start.', 'STEP'));
        return new Promise((resolve, reject) => {
            this.uploadQuery(sftp, querys, 0)
                .then((finish) => {
                if (finish) {
                    log(utils_1.logMsg('query upload finish.', 'STEP'));
                    resolve();
                }
                else {
