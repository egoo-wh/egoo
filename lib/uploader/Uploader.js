...
let {serverPathJoin, sfcall, getConfigURL} = require('../utils');
...
class Uploader extends Handler {
	 ...
	 */
	constructor(localPath, remotePath, isRemoteServer, noCache) {
		super();
		...
		.then(()=>this.ssh.connect(this.serverCfg[k]))
					...
					if (this.serverHash.isNotHashFile(filePath)) {
						...
						return this.serverHash.compare(filePath).then((equal)=>{
							if (!equal) {
								...
								log(LOG_NAME, colors.gray('skip '+_.isArray(rtPath)?rtPath.join(path.sep):rtPath));
							}
						});	
					}
				}
