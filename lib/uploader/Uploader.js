...
let {serverPathJoin, sfcall, getConfigURL, isHttpPath} = require('../utils');
...
class Uploader extends Handler {
	 ...
	 */
	constructor(localPath, remotePath, isRemoteServer, noCache, serverConfigPath, shtmlReplaced) {
		super();
		this.remoteConfKey = isRemoteServer ? 'dist' : 'local';
		...
		// 服务器配置文件地址
		this.serverConfigFilePath = serverConfigPath || getConfigURL(SERVER_CONFIG_FILENAME);
		// this.serverConfigFilePath = path.join(global.rootPath, 'data', SERVER_CONFIG_FILENAME);
	}
	/**
	 * 开始运行
	 * @return {[type]} [description]
	...
	run() {
		...
		.then(()=>{
			// 如果未指定remotePath，则从配置中获取
		 	if (!this.remotePath) {
			 	this.remotePath = this.serverCfg[this.remoteConfKey].path;
			}
			return Promise.resolve();
		})
		.then(()=>this.isLocalPathValid())
		.then(()=>this.ssh.connect(this.serverCfg[this.remoteConfKey]))
		...
		.then(()=>{return !this.noCache?this.serverHash.clear():Promise.resolve()})
		.then(()=>this.commenter.clear())
		.then(()=>this.ssh.close())
		.then(()=>{
			log(LOG_NAME, colors.green('publish success.'));

			// if (!this.isFile) {
			// 	let BASE_URL;
			// 	if (isRemoteServer) { BASE_URL = 'http://preview.egoodev.cn/'; }
			// 	else { BASE_URL = 'http://192.168.1.11/preview/'; }
			// 	log(LOG_NAME, 'preview url: ' + colors.underline(BASE_URL + path.basename(localPath) +'/' + this.indexFileName));
			// }
	...
	loadServerConfig() {
		...
		return new Promise((resolve, reject)=>{
			if (isHttpPath(this.serverConfigFilePath)) {
				try {
					fetch(this.serverConfigFilePath)
					.then((res) => res.json())
					.then((json) => {
						this.serverCfg = json;
						resolve();
				...
				try {
					this.serverCfg = require(this.serverConfigFilePath);
					resolve();
				} catch(err) {
					reject();
				}
