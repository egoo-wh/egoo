...
const log = (0, utils_1.logger)('ServerHash');
...
class ServerHash {
    ...
    async onQueryFile(queryInfo, uploadInfo) {
        ...
        if (this.isNotHashFile(filePath)) {
            ...
            if (!isEqual) {
                ...
                log((0, utils_1.logMsg)('skip: ' + (0, utils_1.logMsg)(filePath, 'PATH')));
            }
        }
    }
    // 从服务器加载并载入hash文件。
    include(sftp, localPath, remotePath) {
        const fileInfo = {
            ...
            serverPath: (0, utils_1.serverPathJoin)(remotePath, hashCfg.file),
            assets: null,
            saved: false
        };
        this.fileInfos[localPath] = fileInfo;
        // 1.load server hash file
        return (0, asyncs_1.sfcall)(sftp.fastGet, sftp, true, fileInfo.serverPath, fileInfo.filePath).then((fileExist) => {
            // 2.include it if file exists.
            if (fileExist) {
                log((0, utils_1.logMsg)('server hash file loaded.', 'STEP'));
                ...
                try {
                    ...
                    log((0, utils_1.logMsg)('include hash file.', 'STEP'));
                }
                catch (err) {
                ...
                log((0, utils_1.logMsg)("server hash file isn't exists.", 'STEP'));
                return Promise.resolve();
            }
        });
    }
    ...
    save(uinfo) {
        ...
        if (fileInfo && fileInfo.assets) {
            return fswriteFile(fileInfo.filePath, JSON.stringify(fileInfo.assets)).then(() => {
                fileInfo.saved = true;
                log((0, utils_1.logMsg)(`save hash file(${fileInfo.filePath}).`, 'STEP'));
    ...
    // 清除hash文件
    clear() {
        log((0, utils_1.logMsg)('remove hash files.', 'STEP'));
