...
class SSIPatch extends patch_1.Patch {
    ...
    /**
       ...
       */
    replaceSSI(res) {
        log(utils_1.logMsg('server side include.', 'STEP'));
                ...
                if (includeFilePath) {
                    ...
                    let p = fslstat(includeFilePath).then((stat) => {
                        return this.getIncludeFileContent(includeFilePath, (contents) => {
                            ...
                            log(utils_1.logMsg(`get include content ${includeFile}`, 'STEP'));
                        });
                    }).catch((err) => {
                        throw (err);
                    });
    ...
    async getIncludeFileContents() {
        log(utils_1.logMsg('get include contents'));
        this.infos.map(info => {
        });
