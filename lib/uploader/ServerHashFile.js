...
let {log, serverPathJoin, sfcall} = require('../utils');
...
class ServerHashFile {
	...
	include(sftp, localPath, remotePath) {
		...
		return sfcall(sftp.fastGet, sftp, true, this.hashServerPath, this.hashFilePath).then((fileExist)=>{
			...
			if (fileExist) {
				log(LOG_NAME, gutil.colors.yellow('server hash file loaded.'));
				...
				try {
					...
					log(LOG_NAME, gutil.colors.yellow('include hash file.'));
				} catch (err) {
					Promise.reject(new Error('require hash file error.'))
				}
				return Promise.resolve();
			} else {
				log(LOG_NAME, gutil.colors.yellow("server hash file isn't exists."));
				return Promise.resolve();
			}
		});
	}
	...
	save() {
		return new Promise((resolve, reject)=>{
			if (this.hashAssets) {
				fs.writeFile(this.hashFilePath, JSON.stringify(this.hashAssets), function(err) {
					if (err) { reject(err); }
					resolve(true);
				})
				log(LOG_NAME, gutil.colors.yellow('save hash file.'));
			} else {
				resolve(false);
			}
		})
	...
	// 清除hash文件
	clear() {
		if (this.hashFilePath) {
			log(LOG_NAME, gutil.colors.yellow('remove hash file.'));
			return new Promise((resolve, reject)=>{
				fs.unlink(this.hashFilePath, (err)=>{
					resolve();
				})
			});
		} else {
