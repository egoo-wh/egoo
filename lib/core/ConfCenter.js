...
const log = (0, utils_1.logger)('ConfCenter');
/**
 * 配置中心
 * 1. 从远程地址下载配置文件。
...
class ConfCenter {
    ...
    async include(configFileName, forceReload = false) {
        if (!this.included) {
            log((0, utils_1.logMsg)('include', 'STEP'));
            await this.createDataDir();
            await this.versionCtrl.init();
            // 配置文件本地路径
            const confPath = (0, utils_1.getLocalURL)(`${configFileName}.json`);
    ...
    async load(configFileName, confPath) {
        log((0, utils_1.logMsg)('load', 'STEP'));
        return new Promise((resolve, reject) => {
            fetch((0, utils_1.getConfigURL)(`${configFileName}.json`)).then(res => {
                if (res.ok) {
