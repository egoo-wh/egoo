...
const log = (0, utils_1.logger)('UserConf');
/**
 * 管理用户配置
 * UserConfig
 * 单例
...
class UserConf {
    ...
    include(confFileName = null) {
        if (!confFileName) {
            this.confPath = (0, utils_1.getLocalURL)(USER_CONFIG_FILENAME);
        }
        else {
            this.confPath = (0, utils_1.getLocalURL)(confFileName);
        }
        if (!this.included) {
            return this.createDataDir().then((exists) => {
                if (!exists) {
                    log((0, utils_1.logMsg)("找不到data文件夹", 'ERROR'));
                    return;
                }
                try {
                    this.assets = require(this.confPath) || {};
                    log((0, utils_1.logMsg)('include user conf.', 'STEP'));
                }
                catch (err) {
                    // no user conf file.
                }
    ...
    createDataDir() {
        let dataDir = (0, utils_1.appRootPathJoin)('data');
    ...
    save() {
        if (this.assets) {
            log((0, utils_1.logMsg)('save user conf.', 'STEP'));
