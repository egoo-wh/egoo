...
const log = utils_1.logger('SSHClient');
...
class SSHClient {
    ...
    constructor(configPath) {
        ...
        this.configFilePath = configPath || utils_1.getConfigURL(SERVER_CONFIG_FILENAME);
    ...
    async loadServerConfig() {
        log(utils_1.logMsg('load server config.', 'STEP'));
        if (utils_1.isHttpPath(this.configFilePath)) {
            try {
    ...
    async cipherDecrypt(contents) {
        ...
        try {
            ...
            const ciphers = cipher_1.default(secret);
     ...
     */
    connect() {
        log(utils_1.logMsg('connecting...', 'STEP'));
        return new Promise((resolve, reject) => {
            let client = new ssh2_1.Client();
            client.on('ready', () => {
                this.client = client;
                log(utils_1.logMsg('ssh connected.', 'STEP'));
                resolve();
            }).on('error', (err) => {
                reject(err);
            }).connect({
        ...
        return new Promise((resolve, reject) => {
            this.client.sftp((err, sftp) => {
                if (err) {
                    ...
                    log(utils_1.logMsg('sftp opened.', 'STEP'));
                    resolve(sftp);
                }
            });
        });
    ...
    close() {
        return new Promise((resolve, reject) => {
            try {
                this.client.end();
                log(utils_1.logMsg('ssh closed.', 'STEP'));
                resolve();
            }
            catch (err) {
                reject(err);
